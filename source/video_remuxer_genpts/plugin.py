...
from video_remuxer_genpts.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.video_remuxer_genpts")
...
def on_worker_process(data):
    ...
    if mapper.container_needs_remuxing(container_extension):
        ...
        # Insert '-fflags +genpts' before the input file arg ('-i') so ffmpeg regenerates
        # missing/invalid timestamps on the source stream before muxing. Without this, some
        # old AVI sources with unset packet timestamps fail to mux into MKV with:
        #   "Can't write packet with unknown timestamp" / error code -22
        if '-i' in ffmpeg_args:
            i_index = ffmpeg_args.index('-i')
            ffmpeg_args = ffmpeg_args[:i_index] + ['-fflags', '+genpts'] + ffmpeg_args[i_index:]
